...
const InfrastructureSection = ({ data }: { data: any }) => {
  return (
    <section className="bg-white py-20 lg:py-28 relative font-sora container-pd">
                ...
                <ul className="space-y-4 flex-grow">
                  {method.details.map((item: string, i: number) => (
                    <li key={i} className="flex items-start gap-3">
                      ...
                      <span className="text-body font-semibold text-slate-600">{item}</span>
                    </li>
                  ))}
                </ul>
              </motion.div>
...
const DashboardSection = ({ data }: { data: any }) => {
  return (
    <section className="bg-[#2776ea] py-20 lg:py-28 text-white relative overflow-hidden font-sora container-pd">
      {/* Background Decor */}
                ...
                <div className="flex-shrink-0 mb-6">
                  <div className="flex  gap-4 items-center">
                    ...
                    <div className="flex flex-col">
                      <h3 className="text-h3 font-bold  leading-tight mb-1">{metric.title}</h3>
                      {metric.subTitle && (
...
const FPAnalyzerSection = ({ data }: { data: any }) => {
  return (
    <section className="bg-slate-50 py-20 lg:py-28 relative font-sora container-pd">
      <div className="mx-auto px-6 max-w-6xl">
        <SectionHeader
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h3 className="text-h3 font-bold text-slate-900 leading-tight">{feature.title}</h3>
                </div>
      ...
      <div className="mx-auto px-6 max-w-6xl relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          <div className="lg:sticky lg:top-32 h-fit">
            ...
            <h3 className="text-h3 text-blue-100 font-bold mb-6 opacity-90">{data.subTitle}</h3>
            <p className="text-body text-blue-50 font-medium leading-relaxed mb-10 opacity-80">{data.description}</p>
            <div className="bg-white/5 border border-white/10 p-8 lg:p-10 rounded-[2.5rem] backdrop-blur-md">
              <h3 className="text-h3 font-bold mb-3 text-[#76ea27] flex items-center gap-2">
                <ArrowUpRight size={18} /> Enterprise Definition
              </h3>
              <p className="text-body text-white/70 font-medium leading-loose">{data.definition.content}</p>
            </div>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-5 mb-5 border-b border-white/5 pb-5">
                    ...
                    <h3 className="text-h3 font-bold text-white group-hover:text-[#76ea27] transition-colors">{cap.title}</h3>
                  </div>
