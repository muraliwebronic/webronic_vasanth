...
export default function ServicesClient() {
  ...
  const isGeoPage = !!serviceData.areaServed;

  const jsonLd = isGeoPage ? {
    "@context": "https://schema.org",
    "@graph": [
      {
        "@type": "LocalBusiness",
        "name": `Webronic ${serviceData.areaServed}`,
        "areaServed": { "@type": "Place", "name": serviceData.areaServed }
      },
      {
        "@type": "Service",
        "name": serviceData.serviceType,
        "provider": { "@type": "LocalBusiness", "name": `Webronic ${serviceData.areaServed}` },
        "areaServed": { "@type": "Place", "name": serviceData.areaServed }
      },
      {
        "@type": "FAQPage",
        "mainEntity": serviceData.faq?.items.map((item: any) => ({
          "@type": "Question",
          "name": item.question,
  ...
  return (
    <main className="min-h-screen bg-white selection:bg-[#2776ea] selection:text-white">
      {isGeoPage && (
        <Script id={`geo-json-${category}`} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
      )}
      <UniversalHero data={serviceData.hero} />

      {isGeoPage && serviceData.localClientNote && (
        <div className="bg-slate-50 border-y border-slate-100 py-8">
          <div className="container-pd mx-auto px-6 md:px-12 lg:px-24">
            <p className="text-slate-600 container-pd mx-auto font-medium leading-relaxed max-w-4xl border-l-4 border-[#2776ea] pl-6">
              {serviceData.localClientNote}
            </p>
          </div>
        </div>
      )}
      ...
      <UniversalTechStack id={serviceData.id} data={serviceData.techStack} />

      <UniversalProcess data={serviceData.process} />

      <UniversalIndustries data={serviceData.industries} />
      {/* Client Success Stories will be added later just leave it be  */}
      ...
      {isGeoPage && serviceData.internalLinks && (
        <div className="container-pd mx-auto px-6 md:px-12 lg:px-24 py-16 border-t border-slate-100 mt-16">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">Explore Related Regions & Services</h3>
          <div className="flex flex-wrap gap-4">
            {serviceData.internalLinks.map((link: any, idx: number) => (
              <Link key={idx} href={link.href} className="px-4 py-2 rounded-lg bg-slate-50 text-slate-600 text-sm font-bold hover:bg-[#2776ea] hover:text-white transition-colors">
                {link.label}
              </Link>
            ))}
          </div>
