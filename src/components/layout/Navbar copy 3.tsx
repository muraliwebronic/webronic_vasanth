...
export default function Navbar() {
  ...
  return (
    <>
      <header
        className={`fixed inset-x-0 top-0 z-[100] transition-all lg:px-20 duration-500 ${scrolled || open ? "py-3" : "py-6"
          }`}
      >
        <nav
          className={`relative mx-auto flex max-w-[95%] lg:max-w-none items-center justify-between px-5 py-2.5 lg:px-[4vw] rounded-2xl transition-all duration-500 ${scrolled || open
                  ...
                  <div className="bg-white/95 backdrop-blur-xl border border-slate-100 rounded-2xl p-[clamp(8px,0.6vw,14px)] shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] flex flex-col gap-1">
                    {productItems.map((item, index) => (
                      <button
                        key={item.name}
                        onClick={() => handleNavClick(item)}
                        className={`flex items-center gap-[clamp(12px,0.9vw,18px)] w-full p-[clamp(10px,0.7vw,16px)] rounded-xl text-left transition-all hover:bg-slate-50 group/item ${index === 0
                          ? "bg-slate-50/80 border-b border-slate-100 mb-1"
                          : ""
                          }`}
                      >
                        <div
                          className={`h-[clamp(32px,2.2vw,44px)] w-[clamp(32px,2.2vw,44px)] rounded-lg flex items-center justify-center transition-colors ${index === 0
